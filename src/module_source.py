...
class py_modulator(Py_Module):
    ...
    def __init__(self, N):
        ...
        sbin_Tx = self.create_socket_out(Tx, "binary_sequences", N, np.int32) # create an output socket for the task Tx

        sbin_Rx = self.create_socket_in (Rx, "binary_sequences", N, np.int32) # create an output socket for the task Tx
        ...
        self.create_codelet(Rx, lambda slf, lsk, fid: slf.bin2img(lsk[simg_Tx], lsk[sbin_Tx])) # create codelet